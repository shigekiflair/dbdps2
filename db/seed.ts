...
async function main() {
  ...
  await db.insert(plans).values({
    ...
    description: "サバイバーの全パークをガチャで1回ずつ引き切るまで終われない耐久企画",
    type: "tracking",
    target: "survivor",
    poolConfig: { source: "perk", role: "survivor", mode: "gacha" },
    inputFields: [],
    outputDisplay: { layout: "gacha_checklist", shareable: true, ogpTemplate: "default" },
    stateModel: "cross_stream_persistent",
  ...
  await db.insert(plans).values({
    ...
    description: "キラーの全パークをガチャで1回ずつ引き切るまで終われない耐久企画",
    type: "tracking",
    target: "killer",
    poolConfig: { source: "perk", role: "killer", mode: "gacha" },
    inputFields: [],
    outputDisplay: { layout: "gacha_checklist", shareable: true, ogpTemplate: "default" },
    stateModel: "cross_stream_persistent",
    progressConfig: { goal: "all_perks_cleared", resetCondition: "manual" },
